        ...
        for type in typesToAdd:
            if (type == "twise"):
                ...
                for t in T_PARAMETER:
                    ...
                    filePath = os.path.join(inputDirectory, ERROR_FILE_PREFIX + type + FILE_NAME_SEPARATOR + "t" + str(t) + ERROR_FILE_SUFIX)
...
def createRanking(typeInformation, allInformation, typesToAdd, toExclude=None):
    result = {}
    print("Typeinformation:", typeInformation)
...
def writeTableToFile(outputFile, labelsOfTypes, typesToAdd, typeInformation, ranking, means=None, meanRanking=None, toExclude=None):
    ...
    file.writelines(addNewLine(["\\begin{adjustbox}{angle=0}", "\t\\begin{tabular}{" + columns + "}", "\t\\toprule", header, midrules]))
    if (len(T_PARAMETER) > 1):
        file.writelines(addNewLine([tLabelLine, spaceBetweenCaseStudies + "[-0.2cm]"]))
    ...
    file.writelines(addNewLine(["\t\t\\bottomrule", "\t\\end{tabular}", "\\end{adjustbox}"]))
            ...
            for type in data[caseStudy]:
                ...
                for t in T_PARAMETER:
                    for result in data[caseStudy][type][t]:
                        file.write(caseStudy + ";" + type + ";" + str(t) + ";" + str(result) + "\n")
...
def writeTestResultsToFiles(outputFile: str, kruskalResult: List[Tuple[str, List[Tuple[str, float]]]], typesToAdd: List[str], labelsOfTypes: List[str], toExclude: List[str], kruskal: bool) -> None:
    if kruskal:
    ...
    with open(testTableFile, 'w') as file:
        file.writelines(addNewLine(["\\begin{tabular}{l r}", "\\toprule", "\\multicolumn{2}{c}{\\texttt{" + testDescription + "}}\\\\", "\\midrule", "& \\textit{p}-value " + NEW_LINE, "\\midrule"]))
...
def main() -> None:
    ...
    writeTestResultsToFiles(output_directory + os.path.sep, kruskalResult, typesToAdd, labelsToAdd, kruskal=True, toExclude=[])

    leveneResult = performRTest(allInformation, kruskal=False, toExclude=TO_IGNORE_RQ2)
    writeTestResultsToFiles(output_directory + os.path.sep, leveneResult, typesToAdd, labelsToAdd, kruskal=False, toExclude=TO_IGNORE_RQ2)
